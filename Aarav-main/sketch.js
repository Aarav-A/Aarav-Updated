...
function draw() {
  ...
  // test = Bodies.circle(200, 200, 50);
  image(Knife, mouseX-5, mouseY-5, 50, 50);
  
/*  if(collideCircleCircle(mouseX, mouseY, 50)){
   console.log("Circles Collided")
}*/  
  ...
  // }
  for (var i = 0; i < fruit.length; i++) {
     if(detectcollision(fruit[i],knife)&&fruitImage == "fruit" + 2){
      console.log("fruit image changed")
...
function detectcollision(f,k){
  fpos=f.body.position 
  kpos=k.position 
  var distance=dist(fpos.x,fpos.y,kpos.x,kpos.y) 
  if(distance<=f.r+k.r){ return true } }
